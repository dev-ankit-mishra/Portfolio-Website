...
const Pagination = lazy(() =>
  import("swiper/modules").then((m) => ({ default: m.Pagination }))
);
const Autoplay = lazy(() =>
  import("swiper/modules").then((m) => ({ default: m.Autoplay }))
);
...
export default function AchievementsSection() {
  ...
  return (
    ...
    >
      ...
      {/* Milestones */}
      <Suspense
        fallback={<div className="text-center">Loading milestones...</div>}
      >
        <div className="flex flex-wrap justify-center gap-6 mb-12 mt-10">
          {[
            ...
            {
              icon: (
                <FaGithub className="text-green-400 text-2xl xl:text-3xl mb-2" />
              ),
            ...
            >
              {item.icon}
              <h3 className={`text-xl xl:text-2xl font-bold ${item.color}`}>
                <CountUp end={item.count} duration={5} enableScrollSpy />+
              </h3>
      ...
      <main className="mx-auto sm:py-0 py-4">
        <Suspense
          fallback={<div className="text-center">Loading certificates...</div>}
        >
          {isMobile ? (
            <Swiper
              pagination={true}
